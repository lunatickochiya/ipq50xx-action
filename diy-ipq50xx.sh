...
function patch_openwrt_openwrt-ipq50xx() {
        for i in $( ls mypatch-openwrt-ipq50xx ); do
            echo Applying mypatch-openwrt-ipq50xx $i
            patch -p1 --no-backup-if-mismatch --quiet < mypatch-openwrt-ipq50xx/$i
        done
        }
function patch_package() {
        for packagepatch in $( ls feeds/packages/feeds-package-patch-openwrt-ipq50xx ); do
            cd feeds/packages/
            echo Applying feeds-package-patch-openwrt-ipq50xx $packagepatch
            patch -p1 --no-backup-if-mismatch < feeds-package-patch-openwrt-ipq50xx/$packagepatch
            cd ../..
        done
        }
function patch_luci() {
        for lucipatch in $( ls feeds/luci/luci-patch-openwrt-ipq50xx ); do
            cd feeds/luci/
            echo Applying luci-patch-openwrt-ipq50xx $lucipatch
            patch -p1 --no-backup-if-mismatch < luci-patch-openwrt-ipq50xx/$lucipatch
            cd ../..
        done
        }
...
function add_ath79_nand_openwrt-ipq50xx_packages() {
echo "$(cat package-configs/ath79-nand-openwrt-ipq50xx-common-iptables.config)" >> package-configs/.config
mv -f package-configs/.config .config
}

function add_ath79_openwrt-ipq50xx_packages() {
echo "$(cat package-configs/ath79-openwrt-ipq50xx-common-iptables.config)" >> package-configs/.config
mv -f package-configs/.config .config
}

function add_openwrt-ipq50xx_packages() {
echo "$(cat package-configs/ipq-common-iptables.config)" >> package-configs/.config
mv -f package-configs/.config .config
}

function add_ramips_mt7620_openwrt-ipq50xx_packages() {
echo "$(cat package-configs/ramips-mt7620-openwrt-ipq50xx-common-iptables.config)" >> package-configs/.config
mv -f package-configs/.config .config
}
...
elif [ "$1" == "ath79-nand-openwrt-ipq50xx" ]; then
autosetver
remove_error_package_not_install
patch_package
patch_luci
patch_lunatic7
add_ath79_nand_openwrt-ipq50xx_packages
elif [ "$1" == "ath79-openwrt-ipq50xx" ]; then
autosetver
remove_error_package_not_install
patch_package
patch_luci
patch_lunatic7
add_ath79_openwrt-ipq50xx_packages
...
patch_luci
patch_lunatic7
add_openwrt-ipq50xx_packages
elif [ "$1" == "ramips-mt7620-openwrt-ipq50xx" ]; then
autosetver
remove_error_package_not_install
patch_package
patch_luci
patch_lunatic7
add_ramips_mt7620_openwrt-ipq50xx_packages
elif [ "$1" == "patch-openwrt" ]; then
patch_openwrt_openwrt-ipq50xx
patch_openwrt
